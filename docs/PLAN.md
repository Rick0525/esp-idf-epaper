...
| ~~**节点 9**：GFX 字体支持~~ ✅ 已完成 | `GFXfont/GFXglyph` 结构 + `epaper_draw_string_gfx` + `get_text_bounds_gfx`，默认带 FreeSansBold9pt7b | 半天 | (x,y)=baseline；MSB-first 与帧缓冲一致；fonts/ 暴露给上层用 `#include "FreeSansBold9pt7b.h"`；后续加字仅复制 .h（改一行 include） |
